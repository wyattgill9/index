...
pub fn render_units_nix(graph: &UnitGraph, options: &RenderOptions) -> Result<String> {
    ...
    out.push_str(
        "{ pkgs, rustToolchain, src, vendorDir ? null, extraNativeBuildInputs ? [], extraEnv ? {} }:\n\n",
    );
    out.push_str("let\n");
...
fn render_rustc_unit(
    ...
    attrs.multiline(
        "installPhase",
        &render_install_phase(unit, &prepared.hashes[index]),
    );
...
fn render_install_phase(unit: &Unit, hash: &str) -> String {
    if unit.is_bin() {
        format!(
            "\
mkdir -p $out/bin
...
mod tests {
    ...
    fn renders_one_derivation_per_build_unit() {
        ...
        assert!(rendered.contains("default = units."));
    }
